...
const checks = [
  ...
  ['handler', "const SMS_DISCLOSURE_VERSION = 'local-line-2026-09-02';"],
  ['contact', 'communications from +19414000165'],
...
if (failures.length) {
  console.error('SMS consent compliance verification failed:');
  failures.forEach(([file, text]) => console.error(`- ${files[file]} is missing: ${text}`));
  process.exitCode = 1;
} else {
  console.log(`SMS consent compliance verification passed (${checks.length} checks).`);
}

...
const providers = [
  {
    _id: 'provider-diana',
    title: 'Diana',
    fullName: 'Diana Morrison',
    lane: 'injectables-medical',
    productionStatus: 'in-production',
    slug: { _type: 'slug', current: 'diana' },
    showOnWebsite: true,
    listingOrder: 10,
    ...
    profileImagePath: '/images/providers/Diana.webp',
  ...
  },
  {
    _id: 'provider-amber',
    title: 'Amber',
    fullName: 'Amber Mingione',
    lane: 'advanced-aesthetics',
    productionStatus: 'in-production',
    ...
    profileImagePath: '/images/providers/amber.webp',
  ...
  },
  {
    ...
    title: 'Brandy',
    fullName: 'Brandy',
    lane: 'classic-facials',
    productionStatus: 'room-rental',
    slug: { _type: 'slug', current: 'brandy' },
    showOnWebsite: true,
    listingOrder: 30,
    publicName: 'Brandy, Licensed Esthetician',
    publicRole: 'Licensed Esthetician',
    summary: 'Brandy is a Licensed Esthetician at House of Rose Aesthetics focused on facial treatments, skin maintenance, peels, and facial waxing.',
    biography: [
    ...
    profileImagePath: '/images/providers/Brandy.PNG',
    digitalCardPath: '/brandy/',
    medicallyDirected: false,
    seo: {
      _type: 'seo',
      metaTitle: 'Brandy | House of Rose Aesthetics',
      metaDescription: 'Meet Brandy, Licensed Esthetician at House of Rose Aesthetics in Punta Gorda, focused on facials, peels, maintenance, and waxing.',
    },
  },
  {
    ...
    title: 'Aundrea',
    fullName: 'Aundrea',
    lane: 'beauty-enhancements',
    productionStatus: 'in-production',
    slug: { _type: 'slug', current: 'aundrea' },
    showOnWebsite: true,
    listingOrder: 40,
    publicName: 'Aundrea',
    publicRole: 'Makeup Artist · Permanent Jewelry Artist',
    summary: 'Aundrea provides professional makeup artistry and permanent jewelry at House of Rose Aesthetics, with each appointment shaped around the person and occasion.',
    biography: [
    ...
    seo: {
      ...
      metaDescription: 'Meet Aundrea, makeup and permanent jewelry artist at House of Rose Aesthetics in Punta Gorda, Florida.',
    },
  },
];

const existing = await client.fetch('*[_id in $ids]{_id}', {
  ids: ['aboutPage', ...providers.map((provider) => provider._id)],
});
console.log(`About/provider seed will upsert ${providers.length + 1} published document(s).`);
console.log(`Existing targets: ${existing.map((document) => document._id).join(', ') || 'none'}`);
...
let transaction = client.transaction().createIfNotExists({ _id: 'aboutPage', _type: 'aboutPage' }).patch('aboutPage', (patch) => patch.set(aboutPage));
for (const provider of providers) {
  const { _id, ...fields } = provider;
  transaction = transaction
    .createIfNotExists({ _id, _type: 'provider', title: fields.title, fullName: fields.fullName })
    .patch(_id, (patch) => patch.set(fields));
}
await transaction.commit();
console.log('Published the About singleton and public provider profile fields.');

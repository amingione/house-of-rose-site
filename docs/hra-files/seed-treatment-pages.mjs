 ...
 *   node packages/studio/scripts/seed-treatment-pages.mjs --apply    # write
 *   node packages/studio/scripts/seed-treatment-pages.mjs --apply --only=iv-hydration
 *
 ...
 *
 *
 *  4. Existing `faqs` are never overwritten — authored FAQs are merged in only
...
const client = createClient({
  projectId: process.env.SANITY_STUDIO_PROJECT_ID ?? '4e7axyi7',
...
async function resolveService(entry) {
  ...
  const docs = await client.fetch(
    `*[_type == "service" && slug.current in $slugs]{ _id, title, "slug": slug.current, status, providerScope, downtime }`,
    { slugs: candidates },
  );
...
async function main() {
  ...
  if (APPLY && !process.env.SANITY_WRITE_TOKEN) {
    log(c.red, '  SANITY_WRITE_TOKEN is not set. Run through scripts/run-with-env.mjs.');
    process.exit(1);
  }
  ...
  if (!APPLY) {
    log(c.dim, '  Re-run with --apply to write.\n');
  }
}

# House of Rose — Charlotte County Competitive Landscape & Service/Pricing Strategy

**Prepared for:** House of Rose | 525 E Olympia Ave, Unit 9, Punta Gorda, FL 33950
**Opening:** June 1, 2026
**Positioning:** Luxury boutique medical spa — clinical precision wrapped in genuine hospitality

---

## 1. Executive Summary

Charlotte County's aesthetic market is bifurcated. On one end sits a small group of legacy operators (C's Medi Spa, Spago Day Spa) anchored in value-tier and dated medi-classic positioning. On the other end sits Skin Sanctuary — the county's only true premium aesthetician-led skin studio and the dominant "Best of Charlotte" winner since 2013. In the middle, a cluster of APRN- and PA-owned injection practices (Sweet Spot MediSpa, Sure Radiance, Florida Lakes Spa, Skin Secrets by Dr. McLaren, FIBB, Nuview Medspa) compete largely on price-per-unit, technical credentialing, and convenience.

What is missing is a single destination that delivers (a) physician/APRN-grade injectables and weight-management protocols, (b) advanced corrective skin care including ProCell Microchanneling and Face Reality Acne Boot Camp, (c) IV hydration and wellness, and (d) lifestyle services like permanent jewelry and lash artistry — all under one elevated, intentional roof. House of Rose fills that gap. The closest premium competitor, Skin Sanctuary, does facials and BioRePeel beautifully but does not offer injectables, IV therapy, GLP-1, ProCell, Face Reality Boot Camp, or permanent jewelry. Sweet Spot offers injectables and GLP-1 but operates in a clinical medical-office aesthetic rather than hospitality-driven luxury.

House of Rose's pricing should sit at the upper edge of the Charlotte County range and align with Naples / Fort Myers premium tier — high enough to signal quality without being aspirationally out of reach for the affluent retiree, second-home, and professional client base in 33950, 33980, 33982, and 33983.

---

## 2. Competitor Summary Table

| Competitor | Location | Owner / Tier | Core Services | Notable Pricing | What They Don't Offer (Gap for House of Rose) |
|---|---|---|---|---|---|
| **Skin Sanctuary** | 6230 Scott St (Bespoke Suites), Punta Gorda | Debbie Fantin, Lead Aesthetician — **Premium Aesthetic / Holistic** | HydraFacial, Wet Diamond/Hydra-Dermabrasion, Microneedling, Nano Channeling (RevitaPen), BioRePeel, Dermaplaning, Specialty Facials, Massage, Lymphatic Drainage, Permanent Makeup, Lash Lift, Brow Lamination, Osmosis Infusion | HydraFacial $235+; Wet Diamond $185+; Microneedling $195+; Nano Channeling $185+; BioRePeel — series of 4 | No injectables, no GLP-1, no IV, no ProCell, no Face Reality Boot Camp, no permanent jewelry, no lash extensions |
| **Sweet Spot MediSpa** | 3691 Tamiami Trl Ste B, Punta Gorda (also Fort Myers) | Amy Ingle, APRN — **Mid-Premium Medical** | Botox, Dysport, Xeomin, Jeuveau, Daxxify; Juvederm, Restylane, RHA, Sculptra, Radiesse; Microneedling/RF; Nordlys IPL; CO2RE Resurfacing; ATTIVA tightening; PRP; Bliss laser lipo; weight loss (semaglutide); permanent makeup | Botox ~$14/unit (per client review); Cancellation fee $60; CareCredit accepted | No advanced corrective esthetics (Face Reality, ProCell), no IV menu beyond weight loss, no permanent jewelry, no lash, weaker on facial luxury experience |
| **Spago Day Spa & Medispa** | 201 W Marion Ave Ste 1311–1314, Punta Gorda | Dr. Michael Stampar, DO — **Mid Medi-Classic / Dated** | Massage ($70–$150), specialty pedicures, full hair salon, facials, Botox, Restylane, Juvederm, Radiesse, Sculptra, laser hair removal, photo facial, Thermage, Lipodissolve, weight loss | Swedish 60 min $70 / 90 min $100; Deep tissue 60 min $85 / 90 min $120; Hot Stone 60 min $115 / 90 min $150 | Brand feels dated; no modern aesthetics layering (no ProCell, no GLP-1 program structure, no Face Reality, no IV menu, no permanent jewelry, no lash extensions) |
| **C's Medi Spa** | 3762 Tamiami Trl Unit G, Port Charlotte | Clara Tovar, Licensed Medical Aesthetician — **Value Tier** | Signature European Facial, Anti-Aging, Vitamin C, PCA Signature, Microdermabrasion, Dermaplaning, Hydro Facial, Mesotherapy/Microneedling, RF Skin Tightening, Body Sculpting, Bio-Slimming Wraps, Massage, Waxing | Facials $60–$110; Microneedling $175; RF Tightening $130; Microderm $80–$90; Brows wax $10; Brazilian $49+; Lipo Laser $120 (12 for $1,000) | No injectables, no GLP-1, no IV, no advanced acne program, no ProCell, no permanent jewelry, no lash extensions; clearly value-positioned |
| **Sure Radiance** | 6210 Scott St Unit 212, Punta Gorda | Jill Stchur, PA-C — **Mid Medical** | Botox, dermal fillers, chemical peels, microneedling, GlacialRx cryo facial, laser, group "tox parties" | Pricing not published; group party model | Small footprint; no facial luxury menu, no acne program, no IV, no GLP-1, no lash, no permanent jewelry |
| **Nuview Medspa** | 6230 Scott St (Bespoke Suites), Punta Gorda | Multi-provider — **Mid Medical** | CoolPeel CO2, Morpheus8, Botox, fillers, IPL, laser hair removal, GLP-1 weight management, IV/IM vitamin therapy | Pricing by consult | Same building as Skin Sanctuary; no permanent jewelry, no Face Reality, no ProCell |
| **Face It Botox Bar (FIBB)** | Punta Gorda location coming soon (also Fort Myers, Estero) | Ryann Lavenia, CRNA, APRN — **Mid-Premium Injection-Focused** | Neurotoxins, fillers, laser, signature treatments, FIBB Funds membership | Membership-driven | Narrow injection focus; not a full-service spa experience |
| **Florida Lakes Spa (Wellen Park)** | Port Charlotte / Wellen Park | Multi-provider — **Mid Medical** | Botox, Juvederm Ultra/Volbella, Lip Flip, microneedling | Pricing by consult | Newer development location; transactional injection menu |
| **Skin Secrets by Dr. Greta McLaren** | Near Port Charlotte | Physician-led — **Mid-Premium Medical** | Botox, Dysport, Restylane, Xeomin, Juvederm, Radiesse | Pricing by consult; FL average $10–$20/unit referenced | Physician-only injectables; not a hospitality-driven spa |
| **Suncoast Skin Solutions** | 25097 Olympia Ave Ste 202, Punta Gorda | Dermatology group — **Mid Medical** | Full dermatology, Botox, Dysport, Juvederm, Restylane, Radiesse, Sculptra, Kybella | Pricing by consult | Dermatology-clinic environment; no spa service layering |
| **Carlisa Health & Wellness** | Punta Gorda | Wellness-focused | Hormone therapy, Biote pellets, IV hydration, PRP, aesthetics | Pricing by consult | Wellness-anchored; no luxury aesthetic positioning |
| **MED 1 Health & Wellness** | Port Charlotte | Men's health/weight loss focus | Semaglutide, Tirzepatide, IV therapy, TRT, HRT | Pricing by consult | Male-focused; no facial/lash/jewelry layering |
| **Desired Medical Solutions** | Punta Gorda | Dr. Salib — **Medical Weight Loss** | Semaglutide, Wegovy, Saxenda, lipotropic injections | Membership model | Single-vertical |

---

## 3. Market Gaps in Charlotte County

The following are services and brand experiences **not currently delivered well** by any single Charlotte County competitor — and where House of Rose can establish category ownership:

1. **A unified luxury spa offering injectables + advanced esthetics + IV + GLP-1 under one roof.** Today, clients drive between Skin Sanctuary (skin), Sweet Spot (injectables), and a separate provider for IV or weight loss. House of Rose is the first to consolidate.
2. **ProCell Microchanneling.** Skin Sanctuary offers Nano Channeling (RevitaPen) and traditional microneedling — but no Charlotte County competitor offers ProCell's bone-marrow-derived stem cytokine protocol specifically.
3. **Face Reality Acne Boot Camp.** No Charlotte County med spa currently markets a structured, certified 12-week clinical acne program. The closest options are dermatology referrals.
4. **A curated IV Hydration menu with NAD+, glutathione, and recovery drips.** Sweet Spot offers IV add-ons in the weight-loss context only; Nuview offers IV vitamins; no one offers a true designed IV bar experience.
5. **Permanent Jewelry as a luxury complement to a med spa visit.** No Charlotte County med spa offers this. Most permanent jewelry in SW Florida is sold at boutique retail (Naples, Fort Myers, Destin).
6. **Lash extensions integrated with medical aesthetics.** Skin Sanctuary offers lash lift/tint only. No premium med spa in Charlotte County offers a full classic-through-mega-volume lash extension menu.
7. **GLP-1 weight management delivered with concierge-level monitoring** rather than transactional refill bookings.
8. **A modern, calm-luxury physical environment.** The county's premium operator (Skin Sanctuary) shares a building with multiple suite-style providers; Sweet Spot is a clinical office; C's and Spago feel dated. House of Rose's standalone unit can deliver a true destination feel.

---

## 4. Recommended House of Rose Service Menu

### Pricing Philosophy
House of Rose sits at the **upper end** of Charlotte County and aligns with Naples / Fort Myers luxury tier — typically 10–25% above Skin Sanctuary on facial work, on par or slightly above Sweet Spot on injectables, and matching Naples / Tampa luxury on IV, ProCell, and Face Reality. No discount language. No "specials." Memberships, packages, and prepaid series replace promotions as the value lever.

---

### A. Injectables & Aesthetic Medicine

| Service | Price | Duration | Description |
|---|---|---|---|
| **Neurotoxin Consultation & Treatment — Botox** | **$15 / unit** (consultation included with treatment) | 30 min | A precise neuromodulator treatment to soften dynamic lines across the forehead, glabella, crow's feet, brow, and jawline. Dosed conservatively for a refreshed, unfrozen result. Treatment is planned around your facial anatomy, not a fixed unit count. |
| **Neurotoxin Treatment — Dysport** | **$5 / unit** (equivalent to ~$15 Botox) | 30 min | A faster-onset neuromodulator option preferred by some clients for natural-looking diffusion. Same clinical assessment, different molecule. |
| **Neurotoxin — Daxxify (extended duration)** | **$22 / unit** | 30 min | A peptide-stabilized neuromodulator designed to last longer between treatments. Recommended for clients who prefer fewer touch-ups annually. |
| **Lip Refinement — Juvederm Volbella XC (½ syringe)** | **$475** | 45 min | A subtle definition treatment using a half syringe of HA filler engineered for fine lines and gentle volume. Designed for first-time clients or maintenance between fuller treatments. |
| **Lip Filler — Full Syringe (Juvederm Ultra XC, Volbella, Restylane Kysse)** | **$795** | 45 min | A full milliliter of hyaluronic acid placed for shape, hydration, and definition. Conservative technique — refinement, not transformation. |
| **Cheek & Mid-Face Filler — Juvederm Voluma / Restylane Lyft** | **$895 / syringe** | 60 min | Mid-face structural support to restore lift and balance. Most clients begin with two syringes; a phased plan is typical. |
| **Jawline & Chin Sculpting** | **$895 / syringe** | 60 min | Definition along the lower face for clients addressing volume loss or structural balance. Number of syringes determined at consultation. |
| **Tear Trough Correction** | **$895 / syringe** | 45 min | Hyaluronic acid placed under the eye to address hollowing. A precise treatment reserved for appropriate candidates. |
| *Add-on: Pre-treatment nitrous (ProNox)* | **+$65** | — | Optional comfort support during injections. |
| *Add-on: Post-treatment LED Phototherapy* | **+$45** | 20 min | Red light to calm and accelerate recovery. |

**Bench note:** Florida averages $12–$20/unit for Botox; SW Florida luxury sits at $15–$22. Sweet Spot is reported around $14. House of Rose at $15 signals premium without overshooting the local ceiling. Filler syringes in FL run $650–$850 (luxury); $795–$895 places House of Rose at the upper-mid range, justified by injector credentialing and consultation depth.

---

### B. The Medical Weight Management Program (GLP-1)

| Service | Price | Duration | Description |
|---|---|---|---|
| **Initial Medical Consultation & Lab Review** | **$195** (credited toward first month if program is started within 14 days) | 60 min | A complete intake including medical history, BMI, contraindication screening, and goal setting. Bloodwork ordered as clinically indicated. |
| **Compounded Semaglutide Program — Monthly** | **$425 / month** (medication, supplies, monthly check-in included) | 20 min monthly | A physician-directed weekly injection program using compounded semaglutide from a U.S. licensed compounding pharmacy. Monthly clinical follow-up ensures dose titration is appropriate. |
| **Compounded Tirzepatide Program — Monthly** | **$595 / month** (medication, supplies, monthly check-in included) | 20 min monthly | A dual GLP-1/GIP receptor program for clients who are clinically appropriate candidates. Monthly clinical follow-up built in. |
| **3-Month Prepaid Semaglutide** | **$1,195** (saves $80) | — | Three months of medication and monitoring, prepaid. |
| **3-Month Prepaid Tirzepatide** | **$1,695** (saves $90) | — | Three months of medication and monitoring, prepaid. |
| *Add-on: Lipo-MIC Injection (weekly)* | **+$45 / injection** or **$160 / month** | — | Methionine, inositol, choline, and B12 to support metabolism. |
| *Add-on: B12 Injection* | **+$30** | — | Energy support, particularly during early dose titration. |

**Bench note:** Florida med spa GLP-1 ranges $279–$800/month. Boutique pricing tier sits $400–$600. House of Rose at $425 (semaglutide) and $595 (tirzepatide) reads as serious medical care, not transactional refills.

---

### C. IV Hydration & Wellness Infusions

| Drip | Price | Duration | Description |
|---|---|---|---|
| **The Reset — IV Hydration** | **$165** | 45 min | Saline, B-complex, B6, B12, and magnesium. The foundational drip for travel, dehydration, or recovery. |
| **The Myers Cocktail** | **$195** | 45 min | The classic clinical formulation — magnesium, calcium, B-complex, B12, and high-dose vitamin C. Foundational support for energy and immune function. |
| **The Immunity** | **$210** | 45 min | Vitamin C, zinc, glutathione, and B-complex. Designed to support immune resilience and post-illness recovery. |
| **The Glow** | **$235** | 60 min | Glutathione, biotin, vitamin C, and B-complex. An infusion built for skin clarity, hair, and nails. |
| **The Recovery** | **$225** | 45 min | Amino acids, B12, magnesium, and electrolytes. Built for athletic recovery, jet lag, and post-event reset. |
| **The Detox** | **$240** | 60 min | Glutathione, alpha lipoic acid, B-complex, and vitamin C. Supports the body's natural detoxification pathways. |
| **NAD+ Infusion — 250mg** | **$365** | 90 min | Cellular regeneration and longevity support. Slow infusion; the entry-point dose. |
| **NAD+ Infusion — 500mg** | **$575** | 2 hr | A higher dose for clients pursuing cognitive clarity, metabolic support, and structured longevity protocols. |
| **NAD+ Loading Series — 4 × 500mg** | **$2,150** (saves $150) | — | The clinical loading protocol for clients beginning NAD+ therapy. |
| *Add-on: Glutathione Push* | **+$75** | — | A standalone antioxidant push, available as an IV add or quick infusion. |
| *Add-on: Toradol* | **+$55** | — | Anti-inflammatory medication, available with medical clearance. |
| *Add-on: Zofran* | **+$45** | — | Anti-nausea medication, available with medical clearance. |
| *Add-on: B12 Boost* | **+$30** | — | Single-shot vitamin B12. |
| *Add-on: Lipo-MIC Boost* | **+$45** | — | Methionine-inositol-choline injection. |

**Bench note:** SW Florida and Tampa benchmarks: hydration $150–$200, Myers $175–$200, Glow/Beauty $165–$235, NAD+ $250–$575 by dose. House of Rose pricing aligns with Tampa luxury IV bars (Tampa Bay Total Wellness, The Blonde Injector in Fort Myers).

---

### D. Advanced Corrective Skin

| Service | Price | Duration | Description |
|---|---|---|---|
| **The Signature Facial** | **$165** | 60 min | A bespoke facial built around your skin assessment that day — cleanse, enzyme exfoliation, extractions, treatment mask, and customized serum infusion. The ideal first visit. |
| **The Refined Facial** | **$215** | 75 min | A deeper version of the Signature, with extended massage protocol, an additional exfoliation pass, and a results-focused mask layered with LED phototherapy. |
| **HydroDermabrasion — Signature** | **$245** | 60 min | Deep cleanse, exfoliation, extraction, and infusion of professional serums via patented vortex technology. No downtime. |
| **HydroDermabrasion — Platinum (with Lymphatic + LED + Booster)** | **$345** | 90 min | The full HydraFacial protocol enhanced with lymphatic drainage, red light phototherapy, and a targeted booster (Britenol, Dermabuilder, or Growth Factor). |
| **BioRePeel CL3** | **$195 / single** | 45 min | A biphasic TCA peel that delivers active ingredients to the dermis without surface peeling or downtime. Most clients see a glow after a single session. |
| **BioRePeel — Series of 4** | **$695** (saves $85) | — | The clinical series for cumulative collagen response, pigmentation, and texture. |
| **Custom Chemical Peel (PCA, Perfect Derma, Glycolic, Lactic)** | **$185** | 45 min | A targeted peel selected for your skin's current needs. Downtime varies by depth. |
| **The Perfect Derma Peel** | **$295** | 45 min | A medium-depth peel incorporating glutathione for pigmentation, fine lines, and texture. Light flaking 3–5 days. |
| **Microneedling — Face** | **$325** | 75 min | Collagen induction therapy using a precision device with sterile, fixed-depth tips. A growth factor serum is layered to enhance the response. Series of 3 recommended. |
| **Microneedling — Face & Neck** | **$425** | 90 min | Extended treatment area for clients addressing décolleté concerns alongside facial rejuvenation. |
| **Microneedling — Series of 3 (Face)** | **$895** (saves $80) | — | The recommended protocol for measurable change in tone and texture. |
| **Microneedling with Exosomes** | **$595** | 90 min | Microneedling layered with an exosome serum for a heightened regenerative response. |
| **ProCell Microchanneling — Face** | **$385** | 75 min | A microchanneling treatment that combines fixed-depth precision channels with ProCell's bone-marrow-derived stem cytokine serum. Minimal downtime, suitable for all skin types. |
| **ProCell Microchanneling — Face & Neck** | **$485** | 90 min | The extended treatment for clients addressing the full upper plane of the face and neck. |
| **ProCell Microchanneling — Series of 3** | **$1,050** (saves $105) | — | The standard protocol for cumulative collagen response. |
| **ProCell Microchanneling — Series of 6** | **$1,995** (saves $315) | — | The advanced protocol for deeper correction; includes complimentary post-treatment LED with each session. |
| *Add-on: LED Phototherapy* | **+$45** | 20 min | Red and near-infrared light to support healing and inflammation. |
| *Add-on: Dermaplaning* | **+$55** | 15 min | Manual exfoliation and vellus hair removal, layered with most facials. |
| *Add-on: Lymphatic Drainage Massage (face)* | **+$65** | 20 min | Manual decongestion technique to reduce puffiness and support clearance. |
| *Add-on: Exosome Serum Layer* | **+$245** | — | A regenerative serum layered into microneedling or microchanneling. |

**Bench note:** Skin Sanctuary anchors Charlotte County premium at HydraFacial $235 and Microneedling $195. House of Rose at $245 / $325 reflects luxury positioning, justified by deeper protocols, longer appointments, and exosome / ProCell options not offered locally. ProCell at $385 sits within the national $300–$500 range. SW FL fellow med spas (Naples, Fort Myers) charge $325–$700 for microneedling.

---

### E. Face Reality Acne Boot Camp

| Service | Price | Duration | Description |
|---|---|---|---|
| **Acne Consultation & First Treatment** | **$185** | 75 min | The clinical intake — skin assessment, full history of products, medications, diet, and lifestyle factors, paired with your first treatment. The customized home regimen is determined here. Required to enter the program. |
| **Face Reality Acne Boot Camp — 12 Weeks** | **$895** (six bi-weekly treatments + ongoing provider support) | — | The complete 12-week certified protocol: six in-office treatments every two weeks, ongoing texting/email support, and skin assessment at each visit. Home-care products billed separately and run approximately $150–$250 across the program. |
| **Bi-Weekly Acne Treatment (in-program)** | Included in Boot Camp pricing above | 45 min | Cleansing, steam, extraction, customized exfoliation, high-frequency, and LED. |
| **Acne Maintenance Treatment (post-program)** | **$165** | 45 min | Once skin is clear, monthly maintenance to protect the result. |
| *Add-on: Back or Chest Treatment* | **+$95** | 30 min | Body acne addressed in tandem with the facial protocol. |

**Bench note:** Face Reality Boot Camps run $500–$865 nationally. Premium markets and dedicated acne-expert practices charge $750–$895 for the full 12-week program. House of Rose at $895 reflects the dedicated certified Acne Expert positioning and is the only structured certified program in Charlotte County.

---

### F. Brow & Lash

| Service | Price | Duration | Description |
|---|---|---|---|
| **Lash Lift & Tint** | **$95** | 60 min | A keratin lift and dye treatment that elevates and darkens your natural lashes for four to six weeks. |
| **Brow Lamination & Tint** | **$95** | 45 min | A texture treatment that softens and shapes the brow, paired with tint and a precision wax. Lasts six to eight weeks. |
---

### G. Waxing

| Service | Price |
|---|---|
| Brow Shape | **$28** |
| Brow Tint | **$22** |
| Brow Shape & Tint | **$45** |
| Lip | **$18** |
| Chin | **$22** |
| Sides of Face | **$28** |
| Full Face | **$65** |
| Underarm | **$32** |

**Bench note:** C's Medi Spa charges $10 for brows, $10 for lip — a clear value-tier signal. House of Rose pricing matches Naples / Fort Myers premium and signals a professional, intentional service rather than a transactional add-on.

---

### H. Permanent Jewelry — The Forever Bar

| Item | Price |
|---|---|
| **14k Gold-Filled Bracelet** | **$95** flat |
| **14k Gold-Filled Anklet** | **$115** flat |
| **14k Gold-Filled Necklace (up to 18")** | **$165** (additional inches at $9/inch) |
| **14k Gold-Filled Ring** | **$65** |
| **Sterling Silver Bracelet** | **$75** flat |
| **Sterling Silver Anklet** | **$95** flat |
| **Sterling Silver Necklace (up to 18")** | **$135** |
| **14k Solid Gold Bracelet — Tier 1 (delicate)** | **$285** |
| **14k Solid Gold Bracelet — Tier 2 (mid-weight)** | **$395** |
| **14k Solid Gold Bracelet — Tier 3 (substantial)** | **$565** |
| **14k Solid Gold Bracelet — Tier 4 (heirloom)** | **$795+** |
| **14k Solid Gold Anklet — Tier 1/2/3/4** | **+$120 above bracelet pricing** |
| **Charm — 14k Gold-Filled** | **$28–$45** each |
| **Charm — Sterling Silver** | **$22–$38** each |
| **Charm — 14k Solid Gold** | **$95–$285** each |
| **Birthstone or Diamond Connector — 14k Solid Gold** | **$145–$345** |
| **Layering — Add a second chain to existing weld (same visit)** | **$25 service fee** + chain price |
| **Re-Weld (broken jump ring, within 14 days)** | **Complimentary** |
| **Re-Weld (after 14 days or removed for medical reasons)** | **$25** |
| **Private Group Welding Event (4+ guests, on-site)** | **$50 booking fee** + per-piece pricing; complimentary champagne pour |

**Bench note:** Florida market: Summie Jewelry (Naples) flat-rate model, Iris Alcoves (Gainesville) starts at $75, Terra Handcrafted (Destin) $65–$99. Naples-tier solid gold runs $132–$370 for delicate-mid-weight bracelets. House of Rose pricing matches Naples / Laguna luxury and is uncontested in Charlotte County.

---

## 5. Package & Bundle Recommendations

These bundles are positioned as curated experiences, never as discounts. Pricing is anchored just below the sum of components to create a clear value rationale without language like "save" or "deal."

| Package | Components | Price | Sum if à la carte |
|---|---|---|---|
| **The Introduction** | Signature Facial + Lash Lift & Tint + Brow Shape | **$295** | $328 |
| **The Reset Day** | Refined Facial + The Reset IV + Brow Shape & Tint | **$425** | $448 |
| **The Glow Protocol** | Glo2Facial Signature + The Glow IV + LED add-on | **$485** | $525 |
| **The Bridal Countdown (8 weeks)** | 2× Refined Facial + 1× BioRePeel + Lash Lift & Tint + Brow Lamination + Permanent Jewelry Bracelet (14k gold-filled) | **$795** | $885 |
| **The Annual Skin Membership** | Monthly facial of choice (Signature or Glo2Facial Signature), 10% off all add-ons, 10% off retail | **$165 / month** | — |
| **The Injectable Insider** | Annual membership: 1 unit of Botox stored monthly toward your next visit, 10% off filler, complimentary post-treatment LED, two priority booking slots per quarter | **$45 / month** | — |
| **Microneedling Series** | 3× Microneedling Face + complimentary LED at each visit + take-home recovery kit | **$995** | $1,110 |
| **ProCell Foundation Series** | 6× ProCell Microchanneling Face + ProCell home aftercare kit + 3 complimentary LED sessions | **$2,295** | $2,575 |
| **The Acne Commitment** | Face Reality 12-Week Boot Camp + 1 BioRePeel at week 8 (when skin is ready) + 90-day product starter set placeholder (products billed separately) | **$995** (program + BioRePeel only) | $1,090 |
| **NAD+ Loading Plus** | 4× NAD+ 500mg + 4× Glutathione push + 4× B12 boost | **$2,495** | $2,720 |
| **The Permanent Jewelry Duo** | Two 14k gold-filled bracelets welded same visit (gift or shared milestone) | **$165** | $190 |
| **The Welding Soirée** | Up to 6 guests, on-site or in-spa, 60-minute event with sparkling rosé, 14k gold-filled welds for first piece, charms à la carte | **$595 hosting fee** + per-piece pricing | — |

---

## 6. Pricing Strategy Notes & Positioning Cues

- **Botox at $15/unit** sits one dollar above the perceived Sweet Spot price point, signaling premium without alienating local clientele. Daxxify at $22/unit captures clients who value extended duration and aligns with the premium SW FL injection market.
- **Filler at $795–$895/syringe** sits in the upper-FL bracket ($650–$1,000) and is consistent with West Palm Beach averages. This protects against the "med spa volume injector" perception.
- **GLP-1 at $425 (semaglutide) / $595 (tirzepatide)** is competitive with the South FL boutique tier ($350–$700) and well above the discount telehealth tier ($199–$299). Bundling labs, monitoring, and clinical follow-up into the price reinforces the "real medicine, not refill mill" position.
- **IV pricing** matches the Tampa luxury and Fort Myers boutique tier (The Blonde Injector, Tampa Bay Total Wellness), and is positioned $20–$50 above standard Florida IV bars.
- **HydraFacial at $245 / Signature at $165** is intentionally $10 above Skin Sanctuary's $235 anchor — a small but deliberate signal of premium. The Refined Facial at $215 fills the white space between the standard and HydraFacial price points.
- **ProCell at $385–$485** owns a category Charlotte County does not currently address.
- **Face Reality Boot Camp at $895** is the only structured 12-week certified acne program in the county. The price positions it as serious clinical work, not a facial series.
- **Permanent Jewelry** functions as both a profitable add-on and a powerful word-of-mouth driver. Group welding events are an ideal soft-launch tool for the 525 E Olympia Ave space.
- **No "save," "deal," "affordable," or "discount" language anywhere** in published menus, social copy, or in-suite collateral. Where bundles offer better economics, the language is structured: "à la carte total $448 · package $425."
- **Memberships replace promotions.** The Skin Membership ($165/mo) and Injectable Insider ($45/mo) build recurring revenue and remove the temptation to discount.

---

## 7. Brand Voice — Service Description Standards

Every service description on the menu, website, and in-suite collateral should follow this tone:

- **Declarative, not promotional.** "A precise neuromodulator treatment to soften dynamic lines" — not "Smooth away years and feel amazing!"
- **Process- and outcome-honest.** Where a series is needed, the description says so. Where downtime exists, the description says so.
- **Clinical vocabulary, plain English.** "Bone-marrow-derived stem cytokine serum" appears alongside "supports your body's collagen response."
- **No exclamation points. No "pamper." No "indulge." No "amazing."**
- **The voice is a quiet authority.** It assumes the reader is intelligent, busy, and discerning, and it treats them accordingly.

---

## 8. Phasing Recommendation for June 1, 2026 Opening

**At launch (Day 1):** Injectables, Signature & Refined Facial, HydraFacial, BioRePeel, Microneedling, Lash Lift & Tint, Brow Services, Waxing, Permanent Jewelry, IV Hydration menu (Reset, Myers, Immunity, Glow, Recovery, Detox, NAD+).

**Phase 2 (months 2–3):** ProCell Microchanneling launch, GLP-1 program launch, Lash Extensions full menu, Membership programs.

**Phase 3 (months 4–6):** Face Reality Acne Boot Camp launch (requires Acne Expert certification timing), Welding Soirée private events, Bridal Countdown package marketing.

This phasing protects the team's bandwidth at opening, allows the core revenue services (injectables, facials, IV) to anchor early reviews, and creates natural marketing waves through the first six months without relying on promotional pricing.